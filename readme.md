...
avl树中每一个节点的左子树和右子树高度差不超过1， 因为极度的平衡，添加/删除过程中容易触发失衡导致旋转操作，比起红黑树更适合读多写少的场景。
```golang
                ┌───────────────────[13(h=3)]────────────────┐
                │                             	             │
        ┌─────[7(h=3)]──────┐                       ┌────[17(h=2)]───┐
...
阶数为order的b树，节点数最大为order, 节点数最小为(order + 1) / 2 - 1（root节点除外）， 子节点数比节点数多+1(叶子节点没有子节点)。
...
阶数为order的b+树，节点数最大为order - 1, 节点数最小为(order + 1) / 2 - 1（root节点除外）， 子节点数比节点数多+1(叶子节点没有子节点)。
...
为什么更多人采用红黑树，因为它更适合频繁的插入/删除。对各种树有一个清晰的认知，能做对更好选择。可以根据自己的需要对着复杂度表看。
